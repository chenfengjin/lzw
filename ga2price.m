...
CostFunction=@(x) -1*onepass2price(x);     % Cost Function
...
VarMax= 150;         % Upper Bound of Variables

VarMin2=1;         % Lower Bound of Variables
VarMax2=65;         % Upper Bound of Variables
%% GA Parameters
...
for i=1:nPop
    ...
    % Initialize Position
    pop(i).Position(1:5)=sort(unifrnd(VarMin,VarMax,1,5));
...
figure
plot(BestPrice(:,1),'LineWidth',2);
hold on
plot(BestPrice(:,2),'LineWidth',2);
xlabel('Iteration');
ylabel('Price');
grid on;
toc