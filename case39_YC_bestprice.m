...
%         disp(quoted_prices)
clear all;
       quoted_prices=[36.18 38.20 38.29 89.56 106.78 1];
...
[ref, pv, ~] = bustypes(bus,gen);%matpower通用函数，创建参考节点，PV 节点和 PQ 节点的节点向量
...
% gencost(5,[4,6,8,10,12])=800;
% gencost(6,[4,6,8,10,12])=800;
% gencost(5,2)=0;
% gencost(6,2)=0;
%报价系数差分处理
Len.Cost=13;
...
for t=1:1:nt
    ...
    con=con+[sum(Pmax.*I(:,t))>=1.1*(sum(Pdtt(:,t)))];%南网系统正备用
...
ses = sdpsettings('verbose',2,'solver','cplex','debug',1);
...
ses2 = sdpsettings('verbose',2,'solver','cplex','debug',1);
...
Fee_PSH=Fee_PSH+sum(LMP(gbus_PSH,t).*(P(5:6,t)-P_pump_PSH(:,t)));
end
...
% LMP24=zeros(nb,24);
% P24=zeros(ng,24);
% P_pump_PSH24=zeros(20,24);
% for t=1:1:24
% LMP24(:,t)=sum(LMP(:,4*t-3:4*t),2)/4;
% P24(:,t)=sum(P(:,4*t-3:4*t),2)/4;
% P_pump_PSH24(:,t)=sum(P_pump_PSH(:,4*t-3:4*t),2)/4;
% Fee_THE=Fee_THE+sum(LMP24(gbus_THE,t).*P24(1:2,t));
% Fee_LNG=Fee_LNG+sum(LMP24(gbus_LNG,t).*P24(3:4,t));
% Fee_PSH=Fee_PSH+sum(LMP24(gbus_PSH,t).*(P24(5:6,t)-P_pump_PSH24(:,t)));
% end
...
C_f_THE=sum(C_f(1:2,:))/2;
C_f_LNG=sum(C_f(3:4,:))/2;
C_f_PSH=sum(C_f(5:6,:))/2;
C_f_Pi_THE=sum(C_f_Pi(1:2,:))/2;
C_f_Pi_LNG=sum(C_f_Pi(3:4,:))/2;
C_f_Pi_PSH=sum(C_f_Pi(5:6,:))/2;

P_PSH_ALL=P_gen_PSH_all-P_pump_PSH_all;
P_PSH_ALL_24=zeros(1,24);
for i=1:1:24
P_PSH_ALL_24(i)=sum(P_PSH_ALL(4*i-3:4*i))/4;
end

%order2 未存
% %广蓄出力
% P_gen_PSH_G=sum(P_gen_PSH(1:8,:));
% %惠蓄出力
% P_gen_PSH_H=sum(P_gen_PSH(9:16,:));
% %清蓄出力
% P_gen_PSH_Q=sum(P_gen_PSH(17:20,:));
% 
% %广蓄抽水
% P_pump_PSH_G=sum(P_pump_PSH(1:8,:));
% %惠蓄抽水
% P_pump_PSH_H=sum(P_pump_PSH(9:16,:));
% %清蓄抽水
% P_pump_PSH_Q=sum(P_pump_PSH(17:20,:));

%电厂LMP
% LMP_G=LMP24(199:206,:);
% LMP_G_ave=sum(LMP_G)/20;
% LMP_H=LMP24(207:214,:);
% LMP_H_ave=sum(LMP_H)/20;
% LMP_Q=LMP24(215:218,:);
% LMP_Q_ave=sum(LMP_Q)/20;
% 
% LMP_G_gen=sum(LMP_G_ave([15:23,55:80]))/35;
% LMP_H_gen=sum(LMP_H_ave([1:2,39:48,52:71,77:80]))/(2+10+20+4);
% LMP_Q_gen=sum(LMP_Q_ave([1:6,46,56:60,65:69]))/(6+1+5+5);
% LMP_G_pump=sum(LMP_G_ave([7:26]))/20;
% LMP_H_pump=sum(LMP_H_ave([8:32,93:96]))/(25+4);
% LMP_Q_pump=sum(LMP_Q_ave([17:27,94:96]))/14;


%画图
% figure;
% plot1=plot(Load2);
% axis([1 nt 0 max(Load2)]);
% hold on;


% figure;
% plot1=plot(sum(Pdtt));
% axis([1 nt 0 max(sum(Pdtt))]);
% hold on;
% %% 输出
obj3=0;%求调频市场目标函数
for t=1:1:nf
   obj3=obj3+sum(Pfu(:,t).*C_f_Pi(:,t));
end
%分别出清
P_PSH_gen_ALL=sum(P_gen_PSH_all)/4;
P_PSH_pump_ALL=sum(P_pump_PSH_all)/4;
